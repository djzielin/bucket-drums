...
feedback_comb_filter::feedback_comb_filter(float sample_rate, float max_delay_seconds)
{
  // printf("initializing feedback comb filter: max %f seconds\n",max_delay_seconds);
...
void feedback_comb_filter::set_delay_time(float dt)
{
   ...
   //rt_printf("current delay is now: %d samples\n",_current_delay);
...
void feedback_comb_filter::set_feedback(float f) //feedback corresponds to the Am parameter in the CCRMA writeup
{
   _feedback=f;
}

void feedback_comb_filter::set_lowpass(float val)
{
	lowpassA=val;
	lowpassB=1.0-val;
}
...
float feedback_comb_filter::tick(float input)
{
   ...
   float delay_store_value=lowpassA*computed + lowpassB*prev_val;
   _delay_line[_write_pos]=delay_store_value;          //store the low passed value in the delay line
